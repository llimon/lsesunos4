	       ...
	       int max_gatherer_bytes)
{
  ...
  for (;;)
  {
    ...
    /* Refresh current time inside the loop */
    gettimeofday(&now_tv, NULL);

    /* Calculate elapsed time in seconds with fractional precision */
    elapsed = (double)(now_tv.tv_sec - gather_start.tv_sec) + 
                     (double)(now_tv.tv_usec - gather_start.tv_usec) / 1000000.0;

    /* Use seed_ext_interval as the baseline, but enforce a safe minimum of 15 seconds 
       so slow system stats (like pstat) never get falsely clipped */
    timeout_limit = (double)seed_ext_interval;
    if (timeout_limit < 15.0) {
        timeout_limit = 15.0;
    }

    if ((gatherer_pid > 0) && (gather_fd != -1) && (elapsed > timeout_limit)) {
      if (debug)
        (void)fprintf(stderr, "Gathering time exceeded (elapsed %.2fs > limit %.2fs), kill process %ld\n",
                    elapsed, timeout_limit, (long)gatherer_pid);
